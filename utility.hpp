...
namespace Utility {
    ...
    inline void set_bits(UInt& data,
    ...
    template <std::unsigned_integral UInt>
    inline void set_bit(UInt& data, bool const write_data, std::uint8_t const write_position) noexcept
    {
        write_data ? (data |= (1U << write_position)) : (data &= ~(1U << write_position));
    }

    template <std::unsigned_integral UInt>
    inline UInt get_bits(UInt data, std::uint8_t const read_size, std::uint8_t const read_position) noexcept
    {
    ...
    template <std::unsigned_integral UInt>
    inline bool get_bit(UInt data, std::uint8_t const read_position) noexcept
    {
        return (data & (1U << read_position)) ? 1U : 0U;
    }
    ...
    resized_bitmap(std::array<std::uint8_t, (OLD_WIDTH * OLD_HEIGHT) / 8> const& bitmap) noexcept
    {
        ...
        for (auto new_y = 0U; new_y < NEW_HEIGHT; ++new_y) {
            for (auto new_x = 0U; new_x < NEW_WIDTH; ++new_x) {
                ...
                bool pixel = get_pixel(bitmap, old_x, old_y);
                set_pixel(resized_bitmap, new_x, new_y, pixel);
            }
        }
